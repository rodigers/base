...
The [rollup node](index.md) has an optional peer-to-peer (P2P) network service to improve the latency between
...
- V4 topic
  ...
  - `payload`: A SSZ-encoded `ExecutionPayload`, always the remaining bytes.
    - _Note_ - the `ExecutionPayload` is modified for the first time in Isthmus. See
      ["Update to `ExecutionPayload`"](../../upgrades/isthmus/exec-engine.md#update-to-executionpayload) in the Isthmus spec.
